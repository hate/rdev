...
// Feature-gated implementation: when `key_names` is enabled, provide the full
// macOS key name generation using HIToolbox/TIS. When disabled, provide a
// minimal stub that never generates names (avoids TIS from tap thread).
...
mod imp {
    ...
    impl Keyboard {
        pub fn new() -> Option<Keyboard> {
            Some(Keyboard { is_main_thread: true, dead_state: 0, shift: false, caps_lock: false })
        }
        pub fn set_is_main_thread(&mut self, b: bool) { self.is_main_thread = b; }
        fn modifier_state(&self) -> ModifierState { if self.caps_lock || self.shift { 2 } else { 0 } }
        pub(crate) unsafe fn create_string_for_key(&mut self, code: i64, flags: CGEventFlags) -> Option<String> {
            let modifier_state = flags_to_state(flags.bits());
            if self.is_main_thread {
                self.string_from_code(code, modifier_state)
            } else {
                QUEUE.exec_sync(move || { self.string_from_code(code, modifier_state) })
            }
        }
        pub(crate) unsafe fn string_from_code(&mut self, code: i64, modifier_state: ModifierState) -> Option<String> {
        ...
        fn add(&mut self, event_type: &EventType) -> Option<String> {
            match event_type {
                EventType::KeyPress(key) => match key {
                    Key::ShiftLeft | Key::ShiftRight => { self.shift = true; None }
                    Key::CapsLock => { self.caps_lock = !self.caps_lock; None }
                    key => { let code = code_from_key(*key)?; unsafe { self.string_from_code(code.into(), self.modifier_state()) } }
                },
                EventType::KeyRelease(Key::ShiftLeft | Key::ShiftRight) => { self.shift = false; None }
                _ => None,
            }
        }
        fn reset(&mut self) { self.dead_state = 0; self.shift = false; self.caps_lock = false; }
    }
    ...
    pub unsafe fn flags_to_state(flags: u64) -> ModifierState {
        ...
        let mut modifier = 0;
        if has_alt != 0 { modifier += 1 << 3; }
        if has_caps_lock != 0 { modifier += 1 << 1; }
        if has_control != 0 { modifier += 1 << 4; }
        if has_shift != 0 { modifier += 1 << 1; }
        if has_meta != 0 { modifier += 1 << 0; }
        modifier
    }
}

#[cfg(not(feature = "key_names"))]
mod imp {
    use super::*;
    use objc2_core_graphics::CGEventFlags;

    pub struct Keyboard { is_main_thread: bool, shift: bool, caps_lock: bool }
    impl Keyboard {
        pub fn new() -> Option<Keyboard> { Some(Keyboard { is_main_thread: true, shift: false, caps_lock: false }) }
        pub fn set_is_main_thread(&mut self, b: bool) { self.is_main_thread = b; }
        #[allow(dead_code)]
        pub(crate) unsafe fn create_string_for_key(&mut self, _code: i64, _flags: CGEventFlags) -> Option<String> { None }
    }
    impl KeyboardState for Keyboard {
        fn add(&mut self, event_type: &EventType) -> Option<String> {
            match event_type {
        ...
        fn reset(&mut self) { self.shift = false; self.caps_lock = false; }
    }
}

pub use imp::Keyboard;
